...
export default async function ImprimirRespuestasPage(props: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-[linear-gradient(180deg,#f3eee6_0%,#f8f5ef_100%)] px-4 py-6 text-slate-900 sm:px-6 sm:py-8">
      <div className="print-report-shell mx-auto max-w-6xl space-y-6">
        <div className="flex items-center justify-between gap-3 rounded-[28px] border border-white/80 bg-white/88 px-5 py-4 shadow-[0_20px_60px_-44px_rgba(16,32,51,0.45)] print:hidden">
          <Link
            href="/admin/respuestas"
            className="inline-flex h-10 items-center justify-center gap-1.5 rounded-full border border-[#d8e1ec] bg-white px-4 text-sm font-medium text-slate-700 transition-colors hover:bg-[#edf3f8] hover:text-[#102033] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#102033] focus-visible:ring-offset-2"
          >
        ...
        <section className="print-report-section overflow-hidden rounded-[36px] border border-[#d9e2eb] bg-[linear-gradient(135deg,rgba(9,19,33,0.98)_0%,rgba(16,32,51,0.96)_58%,rgba(24,50,80,0.92)_100%)] p-6 text-white shadow-[0_40px_120px_-56px_rgba(9,19,33,0.75)] sm:p-8">
          <div className="grid gap-8 xl:grid-cols-[1.18fr_0.82fr]">
            <div className="space-y-5">
              <span className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/8 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.28em] text-[#d8e1ec]">
                <FileText className="h-3.5 w-3.5 text-[#ddc699]" />
                Reporte imprimible
              </span>
            ...
            <div className="rounded-[30px] border border-white/12 bg-white/8 p-5 backdrop-blur">
              <div className="flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.26em] text-[#b8cade]">
                <Sparkles className="h-3.5 w-3.5 text-[#ddc699]" />
                Corte actual
              </div>
              <div className="mt-4 grid grid-cols-2 gap-3">
                <div className="rounded-[22px] border border-white/10 bg-black/10 p-4">
                  <p className="text-[11px] uppercase tracking-[0.2em] text-[#b8cade]">Comentarios</p>
                ...
                <div className="rounded-[22px] border border-white/10 bg-black/10 p-4">
                  <p className="text-[11px] uppercase tracking-[0.2em] text-[#b8cade]">Prom. actividades</p>
                  <p className="mt-2 font-display text-4xl font-semibold tracking-[-0.04em] text-white">
              ...
              <div className="mt-3 rounded-[22px] border border-white/10 bg-black/10 p-4">
                <div className="flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.2em] text-[#b8cade]">
        ...
        <section className="print-report-section grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
          <div className="rounded-[30px] border border-[#d9e2eb] bg-white/92 p-6 shadow-[0_26px_80px_-54px_rgba(16,32,51,0.45)]">
            ...
            <div className="mt-4 space-y-3">
              {filterTags.length > 0 ? (
                filterTags.map((tag) => (
                  <div
                    key={tag}
                    className="rounded-[20px] border border-[#e6ebf1] bg-[rgba(247,250,252,0.86)] px-4 py-3 text-sm text-slate-700"
                  >
                    {tag}
                  </div>
                ))
              ) : (
                <div className="rounded-[20px] border border-[#e6ebf1] bg-[rgba(247,250,252,0.86)] px-4 py-3 text-sm text-slate-700">
          ...
          <div className="rounded-[30px] border border-[#d9e2eb] bg-white/92 p-6 shadow-[0_26px_80px_-54px_rgba(16,32,51,0.45)]">
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-3">
              <div className="rounded-[24px] border border-[#d8e1ec] bg-[rgba(247,250,252,0.88)] p-4">
              ...
              <div className="rounded-[24px] border border-[#e2d4c0] bg-[#fbf8f2] p-4">
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#7b622f]">Respuestas con texto</p>
                <p className="mt-2 font-display text-4xl font-semibold tracking-[-0.04em] text-[#5c4925]">
                  {textResponsesCount}
                </p>
              </div>
              <div className="rounded-[24px] border border-[#d8e1ec] bg-[rgba(247,250,252,0.88)] p-4">
        ...
        {responses.length === 0 ? (
          <div className="print-report-section rounded-[30px] border border-[#d9e2eb] bg-white/92 p-12 text-center shadow-[0_26px_80px_-54px_rgba(16,32,51,0.45)]">
            <AlertCircle className="mx-auto h-10 w-10 text-slate-300" />
            <p className="mt-3 font-display text-2xl font-semibold tracking-[-0.04em] text-[#102033]">
          ...
          <>
                  ...
                  {subjectEntries.map((entry) => (
                    <div key={entry.subject} className="space-y-2">
                      <div className="flex items-center justify-between gap-3">
                        <span className="text-sm font-semibold text-[#102033]">{entry.subject}</span>
                ...
                <div className="mt-5 space-y-3">
                  {topActivities.map((activity, index) => (
                    <div
                      ...
                      className="flex items-center justify-between gap-3 rounded-[22px] border border-[#e6ebf1] bg-[rgba(247,250,252,0.86)] px-4 py-3"
                    >
            ...
            <section className="print-report-section rounded-[30px] border border-[#d9e2eb] bg-white/92 p-6 shadow-[0_26px_80px_-54px_rgba(16,32,51,0.45)]">
              <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-slate-400">
              ...
              <div className="mt-6 space-y-5">
                {responses.map((response, index) => {
                  ...
                  return (
                    <article
                      key={response.id}
                      className="print-avoid-break rounded-[28px] border border-[#d8e1ec] bg-[linear-gradient(180deg,rgba(255,255,255,0.98)_0%,rgba(247,250,252,0.92)_100%)] p-5"
                    >
                      <div className="flex flex-col gap-4 border-b border-[#e6ebf1] pb-4 sm:flex-row sm:items-start sm:justify-between">
                        <div className="space-y-2">
                          <div className="flex flex-wrap items-center gap-2">
                            <span className="inline-flex h-8 min-w-8 items-center justify-center rounded-full bg-[#102033] px-3 text-xs font-semibold text-white">
                              #{responses.length - index}
                            </span>
                            <span className="inline-flex items-center rounded-full border border-[#d8e1ec] bg-white px-3 py-1 text-sm font-semibold text-[#102033]">
                              {response.subject}
                            </span>
                          </div>
                          <p className="text-sm text-slate-500">{formatDate(response.created_at)}</p>
                        </div>
                        <div className="rounded-[20px] border border-[#e2d4c0] bg-[#fbf8f2] px-4 py-3 text-sm text-slate-700">
                          <span className="font-semibold text-[#5c4925]">Actividades de ayuda:</span>{' '}
                      ...
                      {visibleFields.length === 0 ? (
                        ...
                        <div className="mt-4 grid gap-4 xl:grid-cols-2">
                          {visibleFields.map((field) => (
                            <div
                              key={field.key}
                              className="rounded-[22px] border border-[#e6ebf1] bg-white px-4 py-4"
                            >
        ...
        <div className="rounded-[24px] border border-[#d9e2eb] bg-white/88 px-5 py-4 text-sm leading-6 text-slate-500 print:hidden">
